...
<html>
    ...
    <body class="white">
        <div class="container voffset-2">
            ...
            <div id="outer-box">
                <div id="mm">
                    <h1>Change Sub Entries Order</h1>
                    <h2>Drag & drop entries box to change order</h2>
                    <?php
                    $myCon = new dbConfig();
                    $myCon->connect();
                    $query = "SELECT c.cat_name, s.* FROM item_sub_category s "
                            . "LEFT JOIN item_main_category c ON s.cat_code=c.cat_code "
                            . "WHERE s.cat_code!= 0 ORDER BY s.sub_order ASC";
                        ...
                        <form method="post" id="formIDSort" action="">
                            ...
                            <ul id="sortable">
                                <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                                    <li id="<?php echo $row["auto_num"]; ?>" class="ui-state-default">
                                        <span class="glyphicon glyphicon-sort"></span>&nbsp; [<?php echo $row["sub_order"]; ?>] <?php echo '<span class="smallfont text-muted">' . $row['cat_name'] . '--> </span>' . $row["sub_name"]; ?>
                                    </li>
                                    <?php
                                }
                                ?>  
                            </ul>
                            <input type="submit" class="btn btn-sm btn-primary voffset-4 float-right" name="submit" value="Update Sub Entries Order" onClick="saveOrder();">
                        </form>
                    </div>
                </div>
            </div>
